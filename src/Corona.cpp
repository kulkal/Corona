...
namespace
{
	...
	constexpr const char (&kCoronaMeshMagic)[8] = CoronaMeshFormat::Magic;
	// v2 is shared with the offline ufbx importer. Old v1 caches are rebuilt.
	constexpr uint32_t kCoronaMeshVersion = CoronaMeshFormat::Version;
	constexpr uint32_t kCoronaMeshMaterialHasAlpha = CoronaMeshFormat::MaterialHasAlpha;
	constexpr uint32_t kCoronaMeshMaxStringBytes = CoronaMeshFormat::MaxStringBytes;
	constexpr uint32_t kCoronaMeshMaxMaterials = CoronaMeshFormat::MaxMaterials;
	constexpr uint32_t kCoronaMeshMaxMeshes = CoronaMeshFormat::MaxMeshes;
	...
	bool RunMeshImportTool(
		const std::filesystem::path& sourcePath,
		const std::filesystem::path& cachePath)
	{
#if CORONA_PLATFORM_IS_WINDOWS
		const std::filesystem::path meshTool = RuntimePaths::RootDirectory() / L"bin" / L"CoronaMeshImport.exe";
		std::error_code ec;
		if (!std::filesystem::exists(meshTool, ec))
		{
			AppendCpuRuntimeTrace(L"[MeshImport] converter missing: " + meshTool.wstring());
			return false;
		}

		const std::wstring commandLine =
			QuoteCommandLineArgument(meshTool.wstring()) +
			L" --input " + QuoteCommandLineArgument(sourcePath.wstring()) +
			L" --output " + QuoteCommandLineArgument(cachePath.wstring()) +
			L" --force";
		std::wstring mutableCommandLine = commandLine;
		const std::wstring workingDirectory = RuntimePaths::RootDirectory().wstring();
		...
		{
			AppendCpuRuntimeTrace(
				L"[MeshImport] converter launch failed: " + sourcePath.wstring() +
				L" error=" + std::to_wstring(GetLastError()));
			return false;
		}
		...
		CloseHandle(processInfo.hThread);
		CloseHandle(processInfo.hProcess);
		if (exitCode != 0 || !std::filesystem::exists(cachePath, ec))
		{
			AppendCpuRuntimeTrace(
				L"[MeshImport] conversion failed: " + sourcePath.wstring() +
				L" -> " + cachePath.wstring() +
				L" exitCode=" + std::to_wstring(static_cast<unsigned long long>(exitCode)));
			return false;
		}
		...
		AppendCpuRuntimeTrace(
			L"[MeshImport] converted: " + sourcePath.wstring() +
			L" -> " + cachePath.wstring() +
			L", elapsedMs=" + FormatMilliseconds(ElapsedMilliseconds(start, std::chrono::steady_clock::now())));
		return true;
#else
	...
	using CoronaMeshFileHeader = CoronaMeshFormat::FileHeader;
	using CoronaMeshDiskVertex = CoronaMeshFormat::Vertex;
	...
	// A cache is rebuilt by CoronaMeshImport when the source model is newer.
	bool IsCoronaMeshCacheUsable(const std::filesystem::path& cachePath, const std::filesystem::path& sourcePath)
	{
		...
		for (const std::filesystem::path& sidecarPath : MaterialSidecarCandidatePaths(sourcePath))
		{
			...
			const auto sidecarTime = std::filesystem::last_write_time(sidecarPath, errorCode);
			if (!errorCode && cacheTime < sidecarTime)
				return false;
			errorCode.clear();
		}
		return true;
...
shared_ptr<Scene> Corona::LoadBinaryMeshModel(const std::wstring& binaryFileName, const std::wstring& sourceFileName)
{
	...
	for (uint32_t meshIndex = 0; meshIndex < header.MeshCount; ++meshIndex)
	{
		...
		for (const CoronaMeshDiskVertex& vertex : vertices)
			mesh->CpuPositions.emplace_back(vertex.Position[0], vertex.Position[1], vertex.Position[2]);
		mesh->CpuIndices = indices;
...
shared_ptr<Scene> Corona::LoadModel(string fileName)
{
	...
	if (IsCoronaMeshCacheUsable(binaryMeshPath, modelPath))
	{
		if (shared_ptr<Scene> binaryScene = LoadBinaryMeshModel(binaryMeshPath.wstring(), wide))
			return binaryScene;
		AppendCpuRuntimeTrace(L"[LoadModel] binary cache rejected, rebuilding: " + binaryMeshPath.wstring());
	}

	if (HasExtensionI(modelPath, L".cmesh"))
	{
		AppendCpuRuntimeTrace(L"[LoadModel] invalid cmesh source: " + wide);
		return nullptr;
	}
	if (!HasExtensionI(modelPath, L".fbx") && !HasExtensionI(modelPath, L".obj"))
	{
		AppendCpuRuntimeTrace(L"[LoadModel] unsupported model source; expected FBX, OBJ, or cmesh: " + wide);
		return nullptr;
	}
	if (!RunMeshImportTool(modelPath, binaryMeshPath))
	{
		AppendCpuRuntimeTrace(L"[LoadModel] external mesh conversion failed: " + wide);
		return nullptr;
	}
	if (shared_ptr<Scene> binaryScene = LoadBinaryMeshModel(binaryMeshPath.wstring(), wide))
		return binaryScene;
	AppendCpuRuntimeTrace(L"[LoadModel] converter produced an invalid cmesh: " + binaryMeshPath.wstring());
	return nullptr;
