...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData* draw_data, VkCommandBuffer command_buffer)
{
    ...
    if (draw_data->TotalVtxCount > 0)
    {
        ...
        VkResult err = vkMapMemory(v->Device, rb->VertexBufferMemory, 0, VK_WHOLE_SIZE, 0, (void**)(&vtx_dst));
        check_vk_result(err);
        err = vkMapMemory(v->Device, rb->IndexBufferMemory, 0, VK_WHOLE_SIZE, 0, (void**)(&idx_dst));
        check_vk_result(err);
...
{
    if (g_FontMemory != nullptr) return true;
...
bool ImGui_ImplVulkan_CreateDeviceObjects()
{
    ...
    if (!g_DescriptorPool)
    {
        ...
        err = vkCreateDescriptorPool(v->Device, &pool_info, v->Allocator, &g_DescriptorPool);
        check_vk_result(err);
    }
    ...
    err = vkCreateGraphicsPipelines(v->Device, nullptr, 1, &info, v->Allocator, &g_Pipeline);
    check_vk_result(err);
...
bool    ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo* info, VkRenderPass render_pass)
{
    ...
    if (!g_RenderPass)
    {
        // Create the Render Pass
        {
            ...
            vkCreateRenderPass(info->Device, &createInfo, nullptr, &g_RenderPass);
        }
    }
