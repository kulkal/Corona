...
void Filter(Texture2D GIResultSHTex,Texture2D GIResultColorTex, uint2 Pos, inout SH result )
{
	...
	float2 CenterPos = Pos;
	float2 CenterPosHighRes = Pos * GIBufferScale + float2(1, 1);

	float CenterDepth = DepthTex[CenterPosHighRes.xy];
	float CenterZ = GetLinearDepthOpenGL(CenterDepth, ProjectionParams.x, ProjectionParams.y) ;

	float3 CenterNormal = GeoNormalTex[CenterPosHighRes.xy];
	...
	for(int yy = -r; yy <= r; yy++) 
	{
		for(int xx = -r; xx <= r; xx++) 
		{
			...
			float3 SampleDepth = DepthTex[CenterPosHighRes.xy];
			...
			float3 Normal = GeoNormalTex[CenterPosHighRes.xy];
